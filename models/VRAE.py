...
class VAE(Model):
    ...
    def q_z(self, x):
        assert x.shape[1] == 1
        x = torch.squeeze(x)
        output, (hn,cn) = self.q_z_layers(x)
        x = hn.transpose(0,1)
    ...
    def p_x(self, z, x):
        ...
        # translate x and start with zeros:
        zeros = torch.zeros_like(x[:,0,:]).view(x.shape[0],1,self.args.input_size[-1])
        x = torch.cat((zeros,x[:,:-1,:]),1)
        
        # append x to z:
        z = z.view(z.shape[0],1,self.args.z1_size)
        z = z.repeat(1,self.args.input_size[-1],1)
        z = torch.cat((x,z),2)
        
        # run LSTM
        output, (hn,cn) = self.p_x_layers(z)

        x_mean = self.p_x_mean(output).view(-1, np.prod(self.args.input_size))
        if self.args.input_type == 'binary':
            x_logvar = 0.
        else:
    ...
    def forward(self, x):
        ...
        # x_mean = p(x|z)
        x_mean, x_logvar = self.p_x(z_q, x)
