...
for i, mdl in enumerate(models): 
    ...
    for fl in mdl:
        ...
        if 1: #(len(temp1) == T): #and ('permutation_1' in fl): #and ('permutation_4' not in fl) and ('permutation_0' not in fl):
    ...
    M = 20
    plt.plot(np.arange(len(temp1[:M])), temp1[:M], '-' + colors[i] + markers[i], label=lbl)
