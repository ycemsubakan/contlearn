...
def sample_hyperparam_configs(arguments, Nconfigs=100):
    ...
    if (arguments.att == 0) and (arguments.side == 1):
        Kdis = np.random.choice([20, 50, 100], Nconfigs, replace=True).reshape(-1, 1)
    elif (arguments.att == 1) and (arguments.side == 1):
        Kdis = np.random.choice([50, 100, 150], Nconfigs, replace=True).reshape(-1, 1)
    else:
    ...
    ntemp = np.random.choice([100, 200, 250, 300, 500], Nconfigs, replace=True).reshape(-1, 1)
...
def form_mixtures(digit1, digit2, loader, arguments): 
    ...
    for i, (ft, tar) in enumerate(loader):   
        # digit 1
        mask = torch.eq(tar, digit1) #+ torch.eq(tar, 2) #+ torch.eq(tar, 4) + torch.eq(tar, 6) + torch.eq(tar, 8)
        inds = torch.nonzero(mask).squeeze()
        ft1 = torch.index_select(ft, dim=0, index=inds)
        dataset1.append(ft1)

        # digit 2
        mask = torch.eq(tar, digit2) #+ torch.eq(tar, 3) #+ torch.eq(tar, 5) + torch.eq(tar, 7) + torch.eq(tar, 9)
        inds = torch.nonzero(mask).squeeze()
    ...
    dataset_mix = data_utils.TensorDataset(dataset_mix, dataset_mix,
                                           dataset1[:Nmix], dataset2[:Nmix]) 
                                           
    dataset1 = data_utils.TensorDataset(inp1,
                                        dataset1)
    dataset2 = data_utils.TensorDataset(inp2,
                                        dataset2)
                                               

    kwargs = {'num_workers': 1, 'pin_memory': True} if arguments.cuda else {}
    shuffle = True
    loader1 = data_utils.DataLoader(dataset1, batch_size=arguments.batch_size, shuffle=shuffle, **kwargs)
    loader2 = data_utils.DataLoader(dataset2, batch_size=arguments.batch_size, shuffle=shuffle, **kwargs)
    loader_mix = data_utils.DataLoader(dataset_mix, batch_size=arguments.batch_size, shuffle=shuffle, **kwargs)
